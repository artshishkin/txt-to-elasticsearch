...
@Profile("batch-find-warrior-account")
public class BatchExecutionFindWarriorAccountBatchConfig {
        ...
        return steps.get("Find warrior Account and write it to Elasticsearch")
                .tasklet((contribution, chunkContext) -> {
                    ...
                    var savedCount = warriorRepository.findAll()
                            ...
                            .flatMap(warriorAccounts -> Flux.fromIterable(warriorAccounts)
                                    .map(warriorAccount -> SearchRequest.builder()
                                            ...
                                            .city(warriorAccount.getCity().getId())
                                            .build()
                                    )
                                    .collectList()
                                    .flatMap(webApiService::searchUsers)
                                    .map(VkBatchSearchUserResponse::getResponse)
                                    .doOnNext(searchUserResponses -> {
                                        for (int i = 0; i < searchUserResponses.size(); i++) {
                                            VkBatchSearchUserResponse.SearchUserResponse searchResponse = searchUserResponses.get(i);
                                            Set<VkUser> vkUsers = searchResponse.getItems();
                                            WarriorAccount warriorAccount = warriorAccounts.get(i);
                                            warriorAccount.setAccounts(new ArrayList<>(vkUsers));
                                            if (searchResponse.getCount() > 0)
                                                log.debug("Matched Warrior Account: {}", warriorAccount);
                                        }
                                    })
                                    .thenReturn(warriorAccounts)
                            )
